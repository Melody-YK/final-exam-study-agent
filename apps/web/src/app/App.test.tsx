...
describe('App', () => {
  ...
  it('restores an existing course after login without relying on local storage', async () => {
    const user = userEvent.setup()
    vi.spyOn(studyApi, 'currentUser').mockResolvedValue({
      id: 'account-admin',
    ...
    expect(await screen.findByRole('heading', { name: '操作系统' })).toBeInTheDocument()
    expect(localStorage.getItem('study-agent.course-id:account-admin')).toBe('course-existing')
  })
